<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Team') }}: <span class="italic">{{ $team->name }}</span>
        </h2>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('teams.update', $team->id) }}">
                        ...
                        @foreach($team->users as $player)
                            <div class="mb-4 grid grid-cols-2">
                                <div class="w-1/2">
                                    <label class="text-xl text-gray-600" for="players[{{ $player->id }}][id]">
                                ...
                                <div class="w-1/2">
                                    <label class="text-xl text-gray-600" for="players[{{ $player->id }}][position]">
                                        {{ __('Position') }}<span class="text-red-500">*</span>
                                    </label>
                                    <select
                                        name="players[{{ $player->id }}][position]"
                                        id="players[{{ $player->id }}][position]"
                                        class="border-2 border-gray-300 p-2 w-full"
                                    >
